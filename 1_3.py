...
class TextGraph:
            ...
            for target in self.nodes:
                if target != word1 and distances[target] != float('inf'):
                    ...
                    while node is not None:
                        ...
                        node = previous[node]
                    path.reverse()
                    result.append(f"Shortest path from {word1} to {target}: {' -> '.join(path)} (length: {distances[target]})")
            return '\n'.join(result) if result else f"No paths found from {word1} to other nodes."
    ...
    def calc_pagerank(self, word=None, damping=0.85, iterations=100):
        ...
        for _ in range(iterations):
            new_pr = {}
            # Calculate total PR from dangling nodes (nodes with no outgoing edges)
            dangling_pr = 0
            for node in self.nodes:
                # Node is dangling if it has no outgoing edges or isn't in the graph as a source
            ...
            for node in self.nodes:
                ...
                incoming_sum = 0
                for incoming_node in [n for n in self.graph if node in self.graph[n]]:
    ...
    def random_walk(self):
        ...
        try:
            while True:
                ...
                if current_node not in self.graph or not self.graph[current_node]:
                    print(f"Stopping: Node '{current_node}' has no outgoing edges.")
                    break
                ...
                if edge in visited_edges:
                    print(f"Stopping: Repeated edge {current_node} -> {next_node} encountered.")
                    break
        ...
        try:
            with open('random_walk.txt', 'w') as f:
                f.write(walk_text)
            print(f"Random walk path saved to 'random_walk.txt'")
        except Exception as e:
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            save = input("Save graph visualization to file? (y/n): ").lower() == 'y'
            ...
            word2 = input("Enter target word (leave blank for all reachable nodes): ").strip()
            if word2:
            ...
            word = input("Enter a word to get its PageRank (leave blank for top 10): ").strip()
